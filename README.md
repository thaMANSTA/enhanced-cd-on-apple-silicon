...
Use `homebrew` to install `hfsutils`, which allow you to interact with old HFS filesystems.

```
thamansta@thaMANSTAs-Mac-mini ~ % brew install hfsutils
```

Make a mount point, and use the `hmount` command to mount the HFS partition from the EnhancedCD.
...
Notice that it isn't mounted like a usual filesystem: you will need to use the `hfsutils` equivalant commands such as `hls`, `hcopy`, `hpwd`, etc...
...
If you want to copy multiple files at once using the `hcopy` command, you can use `xargs`/
```
...
Sometimes there will be issues with special characters/byte order (In this case an issue with ® or ™️ characters it looks like).  
```
